...
def metal_prepare_chunk_fused_v3(
    ...
    Full fusion of section (a) (q/k L2-norm + KDA gate activation) and
    sections (b)-(g) in one Metal dispatch, reading token-major inputs
    directly. ``q/k/v/g`` arrive as ``[T_total, H, D]`` and ``beta`` as
    ``[T_total, H]`` with ``T_total = n_chunks * CHUNK``. The kernel
    reads them without an MLX-side ``reshape -> transpose(0, 2, 1, 3)``
    step, eliminating an ~1 GB ``ensure_row_contiguous`` copy on every
    forward at bench scale (single-seq H=64).

    Outputs are tile-major ``[n_chunks, H, CHUNK, ...]`` so the
    recurrence kernel consumes them unchanged.
    ...
    Args:
        ...
        valid_tokens_per_chunk: ``[n_chunks]`` int32 — per-chunk valid
            ...
            packed varlen path). The mask preserves
            activate-then-zero-pad varlen semantics so the chunk-local
            cumsum never sees a nonzero gate from a padded position.
